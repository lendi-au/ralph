...
describe("identifyInstance()", () => {
  test("should return instanceId", async () => {
    ...
    const spyGetInstances = sinon.stub(inquireEC2Instances, "getInstances").resolves([]);
...
describe("getInstances()", () => {
  it("should return a list of instances if they exist", async () => {
    ...
    const spyDescribeInstances = sinon.stub().resolves(expectedDescribeInstancesOutput);
    AWSMock.mock("EC2", "describeInstances", spyDescribeInstances);

    const spyExtractInstanceIds = sinon.stub(inquireEC2Instances, "extractInstanceIds");
    spyExtractInstanceIds.withArgs(expectedDescribeInstancesOutput.Reservations).returns(expectedInstances);
...
describe("extractInstanceIds()", () => {
  it("should extract instance Ids as expected", () => {
    ...
    const spyGetInstanceName = sinon.stub(inquireEC2Instances, "getInstanceName");
    spyGetInstanceName.withArgs(flattenReservationListInput).returns("i-1234567890abcdef0 (sample-instance)");
